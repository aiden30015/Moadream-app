...
class ChargeCard extends CustomPainter {
  ...
  void _drawFillArea(Canvas canvas, Rect area) {
    ...
    final fillPaint = Paint()
      ..color = MoaColor.yellow200Opacity
  ...
  void _drawValueLabels(Canvas canvas, Rect area) {
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      final textColor = MoaColor.yellow200;

      // 세로 위치 조정 (포인트 위)
      final yOffset = -16.0;

      _drawText(canvas, formattedValue, point.translate(0, yOffset), textColor);
    }
  }
  ...
  void _drawMonthLabels(Canvas canvas, Rect area, Size size) {
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      _drawText(
        canvas,
        label,
        Offset(point.dx, area.bottom + 15),
        MoaColor.yellow200,
      );
    }
  }
